...
- Fixed progress bar initialization when re-uploading a key (update-only scenario). The confirmation bar now correctly starts at 0/0 (as the total count isn't known until the first confirmation) and updates its total count upon receiving the first `PadConfirmed` event.
