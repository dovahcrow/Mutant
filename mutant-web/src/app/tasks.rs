...
pub struct TasksWindow {
    tasks: Arc<RwLock<Vec<TaskListEntry>>>,
...
impl TasksWindow {
    ...
    pub fn draw_tasks_list(&mut self, ui: &mut egui::Ui) {
        ui.heading(RichText::new("MutAnt Tasks").size(24.0).color(Color32::from_rgb(100, 200, 255)));
        ...
        // Tasks list
        if self.tasks.read().unwrap().is_empty() {
            ...
            egui::Grid::new("tasks_grid")
                ...
                .show(ui, |ui| {
                    ...
                    // Data rows
                    for task in &*self.tasks.read().unwrap() {
                        // Task ID (shortened)
        ...
        if let Some(task) = &*self.selected_task.read().unwrap() {
            ...
            ui.heading(RichText::new("Task Details").size(18.0).color(Color32::LIGHT_BLUE));
            ...
            match &task.result {
                ...
                TaskResult::Result(result) => {
                    ui.label(RichText::new(format!("Completed: {:?}", result)).color(Color32::GREEN));
                },
            }
                ...
                if ui.button("Stop Task").clicked() {
                    ...
                    wasm_bindgen_futures::spawn_local(async move {
                        ...
                        if ctx.stop_task(task_id).await.is_ok() {
                            ...
                            let (task_list, _) = ctx.list_tasks().await;
                            *tasks_list.write().unwrap() = task_list;
